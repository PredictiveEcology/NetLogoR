...
defineModule(sim, list(
  ...
  description = "Translation into R using the NetLogoR and SpaDES packages of the Wolf-Sheep-Predation NetLogo model created by Wilensky (1997)",
  keywords = c("NetLogo", "NetLogoR", "SpaDES", "wolf", "predator", "sheep", "prey", "predation"),
  authors = c(person(c("Sarah", ""), "Bauduin", email="sarahbauduin@hotmail.fr", role=c("aut", "cre"))),
  ...
  parameters = rbind(
    ...
    defineParameter("grassOn", "logical", TRUE, NA, NA, "TRUE to include grass in the model, FALSE to only include wolves and sheep"),
    defineParameter("grassTGrowth", "numeric", 30, 0, NA, "how long it takes for grass to regrow once it is eaten"),
    defineParameter("nSheep", "numeric", 100, 0, NA, "initial sheep population size"),
    defineParameter("gainFoodSheep", "numeric", 4, 0, NA, "amount of energy sheep get for every grass patch eaten"),
    defineParameter("reproSheep", "numeric", 4, 0, 100, "probability in % of a sheep reproducing at each time step"),
    defineParameter("nWolf", "numeric", 50, 0, NA, "initial wolf population size"),
    defineParameter("gainFoodWolf", "numeric", 20, 0, NA, "amount of energy wolves get for every sheep eaten"),
    defineParameter("reproWolf", "numeric", 5, 0, 100, "probability in % of a wolf reproducing at each time step")
  ),
...
doEvent.WolfSheepPredation = function(sim, eventTime, eventType, debug = FALSE) {
  if (eventType == "init") {
    ...
    sim <- scheduleEvent(sim, time(sim) + params(sim)$WolfSheepPredation$.saveInterval, "WolfSheepPredation", "save")
...
WolfSheepPredationInit <- function(sim) {
  ...
  sheep <- turtlesOwn(turtles = sheep, tVar = "energy",
                      tVal = runif(n = params(sim)$WolfSheepPredation$nSheep, min = 0, max = 2 * params(sim)$WolfSheepPredation$gainFoodSheep))
  ...
  wolves <- turtlesOwn(turtles = wolves, tVar = "energy",
                       tVal = runif(n = params(sim)$WolfSheepPredation$nWolf, min = 0, max = 2 * params(sim)$WolfSheepPredation$gainFoodWolf))
...
catchSheep <- function(sim) {
  ...
  sheepWolves <- turtlesOn(world = sim$grass, turtles = sim$sheep, agents = sim$wolves, simplify = FALSE)
...
growGrass <- function(sim) {
  ...
  pBrownCountdown <- of(world = sim$field, var = "countdown", agents = pBrown) # countdown values for the patches equal to 0 (brown)
