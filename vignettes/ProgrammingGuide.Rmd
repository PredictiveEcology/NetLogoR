...
However, you can still plot the world with `plot(nameWorld)` and the agent positions in the world with `points(nameTurtles)` (*i.e.*, "view" in NetLogo), as well as any other plots (*e.g.*, the number of turtles through time) using R plot functions on the R plot interface.
You can also generate outputs on the screen using different functions.
To do so, you need to specifically code for these procedures in the model using R functions and of different packages.
